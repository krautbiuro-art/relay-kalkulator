...
# --- 2. POBIERANIE TRAS (TRIPS) DLA POJAZDU ---
@st.cache_data(ttl=60)
def get_vehicle_trips(api_key, vehicle_id, date_from, date_to):
    # API Ruptela wymaga parametrów from_datetime oraz to_datetime
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            # Sumowanie dystansu i paliwa ze wszystkich przejazdów (trips)
            for trip in trips:
                # W zależności od wersji API klucze to 'distance'/'mileage' oraz 'fuel'/'fuel_used'
                dist = trip.get("distance", trip.get("mileage", 0.0))
